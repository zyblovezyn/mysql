...
BEGIN
	...
	SET @org_no=NULL,@ywzids=NULL,@stationids=NULL,@devType=NULL,@factoryid=NULL,@devModel=NULL,@devstatus=NULL,
	@startdate=NULL,@enddate=NULL,@startline=NULL,@limitline=NULL;
	IF ISNULL(org_no)=FALSE AND LENGTH(org_no)>0 THEN
		SET @org_no=IF(STRCMP(org_no,'8a812897493378a001495677ad086663')=0,CONCAT(' and org.org_no ',base.getInClause(org_no)),''); END IF;
...
-- version 1.0	date:2019-01-03	reason：设备状态查询总数
DROP PROCEDURE IF EXISTS base.sp_getDevWorkStatusDetailCount;
DELIMITER //
CREATE PROCEDURE base.sp_getDevWorkStatusDetailCount(org_no CHAR(40),ywzids MEDIUMTEXT,volids MEDIUMTEXT,
stationids MEDIUMTEXT,devType CHAR(32),factoryid CHAR(50),devModel CHAR(50),devstatus CHAR(10),
startdate VARCHAR(20),enddate VARCHAR(20))
BEGIN

	SET @sqlstr=CONCAT('SELECT 	count(distinct dev.id) as total						 
						FROM
							cisp_dev.v_all_dev dev
						left join base.tf_dev_currentstatus devstatus
							on dev.id = devstatus.dev_id
...
CALL base.sp_getDevWorkStatusDetail('8a812897493378a001495677ad086663','','','','','','','','','','0','30000');
CALL base.sp_getDevWorkStatusDetailCount('8a812897493378a001495677ad086663','','','','','','','','','');
...
SELECT COUNT(DISTINCT id) FROM cisp_dev.v_all_dev  